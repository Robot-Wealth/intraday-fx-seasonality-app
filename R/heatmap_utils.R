...
compose_facet_year_heatmaps <- function(performance_df, tickers, timezone, detrend = FALSE, hour_offset = 0) {
  ...
  for(i in seq_along(tickers)) {
    p <- heatmap_facet_year_plot(df, tickers[i], timezone, detrend, hour_offset, c(ir_range$min, ir_range$max))
...
compose_facet_asset_heatmaps <- function(performance_df, tickers, year_subsets, timezone, detrend = FALSE, hour_offset = 0) {
  ...
  for(i in seq_along(year_subsets)) {
    p <- heatmap_facet_asset_plot(df, year_subsets[i], timezone, detrend, hour_offset, c(ir_range$min, ir_range$max))
